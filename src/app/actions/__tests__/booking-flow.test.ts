...
beforeAll(async () => {
  // Two Test rows: LabTestOffering has a (labId, testId) unique constraint,
  // and both activeOffering and inactiveOffering live on the same lab.
  const [test, test2] = await Promise.all([
    prisma.test.create({ data: { name: `Booking Test ${Date.now()}`, category: "Blood", sampleType: "Serum" } }),
    prisma.test.create({ data: { name: `Booking Test Inactive ${Date.now()}`, category: "Blood", sampleType: "Serum" } }),
  ]);
  testId = test.id;
  testId2 = test2.id;
  ...
  [activeOffering, inactiveOffering, pendingLabOffering] = await Promise.all([
    ...
    prisma.labTestOffering.create({ data: { labId: pendingLab.id, testId, price: 60, turnaroundHours: 24, active: true } }),
  ]);
...
afterAll(async () => {
  await prisma.labTestOffering.deleteMany({ where: { testId: { in: [testId, testId2] } } });
  await prisma.user.delete({ where: { id: patient.id } });
  await prisma.lab.deleteMany({ where: { id: { in: [approvedLab.id, pendingLab.id] } } });
  await prisma.test.deleteMany({ where: { id: { in: [testId, testId2] } } });
});
