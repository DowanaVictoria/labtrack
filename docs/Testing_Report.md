...
All 44 pass as of 2026-08-13 (`npm test`). `npx tsc --noEmit` and `npm run lint` are both clean, including the new test files.
...
Full list of all 44 cases is in the test files themselves (each `it(...)` description is written to double as a test-log entry).

## 4. Defects found and corrected

| Defect | Found via | Corrective action |
|---|---|---|
| `todo.md` claimed platform-admin test-catalog CRUD (`createTest`/`updateTest`/`deleteTest`) was implemented and verified; the code doesn't exist. `docs/SRS.md` §5's "Change note (v2.1)" shows this scope (FR23) was deliberately removed, but the checklist line was never updated to match. | Discovered while scoping which Server Actions the tenant-isolation suite needed to cover — grepped for the claimed file/functions and found nothing. | Corrected the stale `todo.md` line (struck through, with an explanation) rather than writing tests against code that doesn't exist. No functional defect — a documentation-accuracy one. |

No functional defects were found in the code itself during this round of testing — every case listed above passed on the first correctly-written attempt (a few tests initially had bugs in the *test* itself — wrong `Decimal.toString()` formatting expectation, a `(labId, testId)` unique-constraint collision between two fixture offerings, `vi.mocked()` fighting an unrelated NextAuth middleware type overload — all fixed in the test code, not the application code).
...
Still open: formal user acceptance testing and a usability check — both need a human perspective, not something to assert in code. See `docs/Technical_Debt_Plan.md` for known limitations distinct from untested-but-working functionality.
