...
- **Impact:** Growing, silent inventory loss — every cancellation permanently removes a bookable slot. Low severity at pilot scale (few labs, few slots/day), compounds over time.
- **Priority:** Scheduled.
...
- **Cause:** `todo.md`'s original placeholder for this line read "no email/SMS notifications (in-app status only)" — that was already stale by the time Phase 4 started; email was built during Phase 3. Corrected here.
...
- **Debt:** `Sample` (`prisma/schema.prisma`) tracks `collectedAt`, `collectedByStaffId`, and free-text `notes` — no structured result values, no file/PDF attachment for an actual lab report.
...
- **Impact:** A patient sees status progress ("Completed") but not their actual result through the platform — they'd still need to collect it from the lab directly. This is the single biggest functional gap relative to a real product.
