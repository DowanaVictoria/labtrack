...
FR IDs referenced throughout the Use Case Points calculation below map to the following requirements, defined in full (with priority) in `SRS.md` §5:

| ID | What it covers |
|---|---|
| FR1 | Patient can register an account |
| FR2 | User can log in / log out (all roles) |
| FR3 | Patient can search the standardized test catalog by name/category |
| FR4 | Patient can view all labs offering a matched test, with price, location, and turnaround time (comparison view) |
| FR5 | Patient can filter/sort comparison results by price, location, or turnaround time |
| FR6 | Patient can view a lab's profile (name, address, contact, offered tests) |
| FR7 | Patient can book an appointment with a specific lab, for a specific test offering and slot |
| FR8 | System displays prep instructions before booking confirmation |
| FR9 | Patient can view their own appointments and status across all labs they've booked with |
| FR10 | Patient can cancel an upcoming appointment |
| FR11 | Patient can reschedule an upcoming appointment |
| FR12 | Lab staff can log in, scoped to their own lab |
| FR13 | Lab staff can view their lab's queue of today's/upcoming appointments |
| FR14 | Lab staff can mark a sample as collected (records status and timestamp) |
| FR15 | Lab staff can advance an appointment/sample's status along the defined pipeline |
| FR16 | Lab staff can view a single patient's appointment detail (within their own lab only) |
| FR17 | Lab admin can register their lab on the platform (submits a profile for platform admin approval) |
| FR18 | Lab admin can manage their lab's profile (name, address, contact info) |
| FR19 | Lab admin can manage staff accounts for their lab |
| FR20 | Lab admin can add/edit/deactivate their lab's test offerings (select from the platform test catalog; set price, turnaround time, prep instructions) |
| FR21 | Lab admin can view basic operational statistics for their own lab |
| FR22 | Platform admin can review and approve or reject new lab registrations |
| FR23 | *Removed in v2.1* (was: platform admin manages the global standardized test catalog). See `SRS.md` §5 change note; the ID is retained but unused. |
| FR24 | Platform admin can suspend or reinstate a lab |
| FR25 | Platform admin can view platform wide operational statistics (labs, bookings, tests) |
| FR26 | System prevents two appointments from being booked into the same lab's same slot |
| FR27 | System enforces strict multi tenant data isolation: lab staff/admin can only ever access their own lab's data |
| FR28 | *Added in v2.2, post delivery* (not part of the original UCP estimated scope below): lab admin can add a new test to the shared platform catalog when the one they need doesn't already exist |
...
- **T11 Security objectives (rated 5, up from 4):** multi tenant data isolation (NFR2, NFR7) is a hard requirement, since a leak here exposes one lab's patient data to another lab, not just a routine access control bug.
...
- **Book Appointment** remains the single Complex use case (15 pts), but is now materially riskier than in a single tenant design because slot availability checks and clash prevention must be correctly scoped per lab, which kept it Must have with an explicit note that it needs the most testing attention.
...
- The jump from a single tenant UCP of ~59 to a multi tenant UCP of ~120, roughly 2.0 times, not a modest increment, is the direct, traceable consequence of adding a fourth actor and the onboarding/comparison use cases; this is presented explicitly so the effort implication of the multi tenant pivot is visible rather than absorbed silently into a schedule.
